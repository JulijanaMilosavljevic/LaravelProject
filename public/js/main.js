window.onload=function() {
    $('#cart').hide();
    $("body").on("click", "#prod .fa-heart-o", addProdToWishlist);
...
function showWishlist(products){
    if(products){
        ...
        var sum = 0;
        if(length > 0){
            var text = "<table><p id='wishlistHeadline'>My Wishlist <span class='fa fa-heart-o'></span></p>";
            ...
            $('#cart').html("<h1>Your dont't have products in wishlist</h1><input type=\"button\" value=\"Close\" id=\"closeCart\"/>");
            $('#closeCart').click(function(){
                $('#cart').fadeOut();
            });
        }
...
function printProduct(products, shouldPrintPag){
    ...
    for(var product of products) {
        ...
        if(i <=12){
            ...
            if (hot) {
                text += `<div class='hot'>Sale</div>`;
            }

            text += `<span class='fa fa-heart-o'></span><img class='prodPic'alt = '${product.alt}' src = '${urlPics}/${product.picName}'/><div class = 'prodData' ><h2 class = 'prodName' >${product.name}</h2><span class='prodPrice'>${product.price}<span class='dollar'> $ </span></span>`;
            if (product.oldPrice > 1) {
                text += `<del>${product.oldPrice}</del>`;
            }
            text+=`</div></div>`;
